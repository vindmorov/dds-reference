...
function LandingPage({ onStart }: { onStart: () => void }) {
  return <LandingReference onStart={onStart} />;
}
...
function FigmaUploadPage({ onBack, onContinue, initialFile = null, initialSummary = false, initialContext = '' }: { onBack: () => void; onContinue: (file: File, context?: string) => void; initialFile?: File | null; initialSummary?: boolean; initialContext?: string }) {
  const initialContextParts = initialContext.split(';').map(part => part.trim());
  const initialBusiness = initialContextParts[0] ?? '';
  const initialOtherBanks = initialContextParts.find(part => part.startsWith('Счета в других банках:'))?.replace('Счета в других банках:', '').trim() || 'Нет';
  const initialOtherEntities = initialContextParts.find(part => part.startsWith('Другие юридические лица:'))?.replace('Другие юридические лица:', '').trim() || 'Нет';
  const [selectedFile, setSelectedFile] = useState<File | null>(initialFile);
  const [uploadStartedAt] = useState(() => formatTime(new Date()));
  const [error, setError] = useState('');
  const [assistantMessage, setAssistantMessage] = useState('');
  const [assistantTurns, setAssistantTurns] = useState<{ role: 'user' | 'assistant'; text: string; time: string }[]>([]);
  const [contextStartedAt, setContextStartedAt] = useState(initialSummary ? formatTime(new Date()) : '');
  const [assistantPending, setAssistantPending] = useState(false);
  const [contextStarted, setContextStarted] = useState(initialSummary);
  const [analysisStage, setAnalysisStage] = useState<'upload' | 'processing' | 'summary'>(initialSummary ? 'summary' : 'upload');
  const [business, setBusiness] = useState(initialBusiness);
  const [customBusiness, setCustomBusiness] = useState('');
  const [otherBanks, setOtherBanks] = useState(initialOtherBanks);
  const [otherEntities, setOtherEntities] = useState(initialOtherEntities);
  ...
  useEffect(() => {
    if (analysisStage !== 'summary' || !contextStarted) return;
    setAssistantTurns(turns => turns.some(turn => turn.role === 'user' && turn.text === 'Продолжить') ? turns : [...turns, { role: 'user', text: 'Продолжить', time: formatTime(new Date()) }]);
  }, [analysisStage, contextStarted]);
  ...
  const sendAssistantMessage = async () => {
    ...
    setAssistantMessage('');
    setAssistantTurns(turns => [...turns, { role: 'user', text: message, time: formatTime(new Date()) }]);
    setAssistantPending(true);
    try {
      const answer = await askAssistant(message);
      setAssistantTurns(turns => [...turns, { role: 'assistant', text: answer, time: formatTime(new Date()) }]);
    } catch (requestError) {
      setAssistantTurns(turns => [...turns, { role: 'assistant', text: requestError instanceof Error ? requestError.message : 'Не удалось получить ответ. Попробуйте ещё раз.', time: formatTime(new Date()) }]);
    } finally {
      setAssistantPending(false);
    }
  };
  const handleMainBack = () => {
    if (analysisStage === 'summary') {
      setAnalysisStage('upload');
      setAssistantTurns(turns => turns.filter(turn => !(turn.role === 'user' && turn.text === 'Продолжить')));
      return;
    }
    if (contextStarted) {
...
type UploadChatMessage = { role: 'user' | 'assistant'; text: string; time: string };
...
function CategoryChatHistory({ messages }: { messages: UploadChatMessage[] }) {
  return <>{messages.map((message, index) => message.role === 'user' ? <div className="categories-user-message" key={`${message.role}-${index}`}>{message.text}<small>{message.time}</small></div> : <div className="categories-ai-message categories-history-message" key={`${message.role}-${index}`}><strong><span>А*</span>Ассистент</strong><p>{message.text}</p><small>{message.time}</small></div>)}</>;
}
...
function CategorySetupPage({ onBack, onContinue, fileName, context, chatHistory }: { onBack?: () => void; onContinue: () => void; fileName?: string; context?: string; chatHistory: UploadChatMessage[] }) {
...
export default function App() {
  ...
  const [uploadChatHistory, setUploadChatHistory] = useState<UploadChatMessage[]>(() => readStored<UploadChatMessage[]>('dds-upload-chat-history', []));
  const [returnToUploadSummary, setReturnToUploadSummary] = useState(false);
  ...
  if (productStage === 'upload') return <><AppHeader className="upload-shared-header" step={2} sectionLabel="Загрузка" /><FigmaUploadPage onBack={() => setProductStage('account')} initialFile={importFile} initialSummary={returnToUploadSummary} initialContext={assistantContext} onContinue={(file, context) => { const reviewItems = financeRows.slice(0, 5).map((row, index) => ({ id: `preview-${row.id}`, date: `2026-0${index + 1}-15`, amount: row.data.jan, description: row.description, counterparty: 'Контрагент не определён', suggestedCategory: row.name, status: 'pending' as const, category: row.name })); const stats = { total: 1248, auto: 1036, review: 212, unresolved: 100, period: 'Январь — Июнь 2026', fileName: file.name, reviewItems }; const importedRows = buildAnalyticsRows(reviewItems); const messageTime = formatTime(new Date()); setUploadChatHistory([{ role: 'user', text: `Загрузите файл: ${file.name}`, time: messageTime }, { role: 'assistant', text: 'Файл принят. Начинаю разбор операций и уточняю детали о вашем бизнесе.', time: messageTime }, { role: 'user', text: `Контекст бизнеса: ${context || assistantContext || 'не указан'}`, time: messageTime }, { role: 'assistant', text: 'Данные готовы к анализу. Я обработал выписку и подготовил операции.', time: messageTime }, { role: 'assistant', text: 'Всё ли категории удалось распознать верно? Если необходимо, добавьте свои.', time: messageTime }]); setImportFile(file); setAssistantContext(context || assistantContext); setImportStats(stats); setAnalyticsRows(importedRows); setAnalyticsInsights(buildDynamicInsights(importedRows)); setReturnToUploadSummary(false); setProductStage('category-setup'); }} /></>;
  ...
  if (productStage === 'category-setup') return <><AppHeader step={6} sectionLabel="Категории" /><CategorySetupPage chatHistory={uploadChatHistory} fileName={importFile?.name} context={assistantContext} onBack={() => { setReturnToUploadSummary(true); setProductStage('upload'); }} onContinue={() => setProductStage('transaction-review')} /></>;
